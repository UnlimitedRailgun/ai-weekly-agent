...
`Research -> Curate -> Analyze -> Write Markdown -> Save locally`
...
3. **Analyze:** Explain each selected development using only supported facts. Distinguish source-reported facts from explanatory interpretation.
4. **Write Markdown:** Produce a readable weekly report organized by topic, with source links close to the claims they support.
5. **Save locally:** Raw research artifacts belong under `data/raw/`; completed weekly reports belong under `reports/`.
...
Add production modules under `src/ai_weekly_agent/` and mirror that layout under `tests/` (for example, `src/ai_weekly_agent/research.py` and `tests/test_research.py`). Keep research, curation, analysis, Markdown rendering, and local persistence as separate concerns, without turning them into a framework. Put reusable prompts or Markdown templates in `assets/` and one-off maintenance utilities in `scripts/`.
...
There is not yet a dependency manifest or build command. When dependencies are formalized, record runtime and development tools in `pyproject.toml` (preferred) or pinned requirements files, declare support for Python 3.11+, and update this guide with the canonical install and run commands.
